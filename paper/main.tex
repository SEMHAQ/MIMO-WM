...
梯度MPC使用Adam优化器进行30次迭代; CEM-MPC使用$K{=}256$个候选序列、$N_e{=}32$个精英样本、5轮迭代. 预测时域$H{=}10$. 结果如表~4所示.

\noindent\begin{minipage}{\columnwidth}
\begingroup\centering
{\CJKfamily{kai} 表~4~~MPC控制性能对比(Humanoid)}\\
...
Mamba & 1657.5 & 0.6\\
S4D & 1683.9 & 0.6\\
\textbf{MIMO-WM} & 2511.9 & 0.4\\
\bottomrule
\end{tabular}}
\par\endgroup
\end{minipage}

表~4表明, 基于梯度的MPC因反向传播开销导致控制频率普遍偏低(0.2--0.8Hz), 远低于典型MPC控制需求(10Hz). 其中S4D-WM和Mamba-WM的回路时间约5.8s, 瓶颈在于卷积模式下50次反向传播的计算开销.
...
\begin{minipage}{\columnwidth}
\centering
{\CJKfamily{kai} 表~5~~CEM采样式MPC控制性能(Humanoid)\\
...
Mamba & 103.6 & 9.4 \\
S4D & 111.3 & 7.3 \\
\textbf{MIMO-WM} & 153.8 & 6.5 \\\bottomrule
\multicolumn{3}{@{}l@{}}{\fontsize{7pt}{8.5pt}\selectfont 注: $^{\text{a}}$ms; $K{=}100$, $H{=}10$, 1轮迭代, GPU批量并行; 5次独立试验均值.}
\end{tabular}}
\end{minipage}
\par\endgroup

表~5表明, CEM-MPC相比梯度MPC(表~4)可将控制频率提升1--2个数量级.
LSTM和GRU达到20Hz以上, 满足典型MPC控制需求(10Hz);
Mamba达到9.4Hz, 接近实时需求;
MIMO-WM达到6.5Hz, S4D达到7.3Hz.
各方法MPC性能对比如图~6所示.
...
MPC实验表明, CEM采样式MPC可将控制频率提升1--2个数量级, LSTM和GRU达到20Hz以上.
