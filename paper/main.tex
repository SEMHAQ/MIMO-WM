...
人形机器人作为具身智能的重要载体, 其高维度、强非线性及欠驱动特性使得"感知--决策--控制"的闭环实现极具挑战\textsuperscript{\cite{2}}.
其中, 世界模型(world model)作为环境的内部表征, 能够预测系统在给定动作下的未来状态,
是实现模型预测控制(MPC)和规划决策的关键组件\textsuperscript{\cite{3}}.

近年来, 基于深度学习的世界模型取得了显著进展.
基于循环神经网络(RNN)的方法\textsuperscript{\cite{4}}虽然能够建模时序依赖, 但存在梯度消失和长程依赖捕捉困难的问题.
基于Transformer的方法\textsuperscript{\cite{5}}通过自注意力机制有效解决了长程依赖问题,
但其二次计算复杂度$O(T^2)$在长序列场景下带来显著的计算开销.
在具身智能领域, RT-2\textsuperscript{\cite{8}}等大规模视觉-语言-动作模型展示了世界模型在机器人控制中的巨大潜力,
...
S4D\textsuperscript{\cite{22}}进一步简化了对角SSM的参数化方案, 在保持性能的同时降低了实现复杂度.
Mamba\textsuperscript{\cite{7}}引入选择性扫描机制, 使SSM能够根据输入内容动态调整状态更新策略,
在语言建模任务上首次超越了同等规模的Transformer模型.
Mamba-2\textsuperscript{\cite{11}}进一步揭示了SSM与注意力机制之间的对偶关系,
为统一理解序列建模方法提供了新的理论视角.

然而, 将SSM应用于具身智能领域的世界模型构建尚处于起步阶段.
现有工作主要集中在视觉感知和语言理解领域,
而在机器人状态预测和控制方面的探索相对有限.
近期, Mamba策略网络\textsuperscript{\cite{40}}和SSM轨迹预测\textsuperscript{\cite{41}}等工作开始将SSM引入机器人领域,
...
世界模型的概念最早可追溯到Karl Friston的主动推理理论\textsuperscript{\cite{25}},
其核心思想是智能体通过构建环境的内部模型来进行预测和规划.
在深度学习时代, Ha和Schmidhuber\textsuperscript{\cite{3}}提出了基于变分自编码器(VAE)和循环神经网络的世界模型框架,
通过学习压缩的环境表征来实现策略学习.
Hafner等人\textsuperscript{\cite{12}}提出了Dreamer系列方法, 利用潜在动力学模型在想象(imagination)中进行策略优化,
在多个连续控制基准上取得了领先性能.
DreamerV3\textsuperscript{\cite{26}}进一步统一了离散和连续动作空间的处理, 展示了世界模型的通用性.

在机器人领域, Michels等人\textsuperscript{\cite{13}}探索了利用世界模型进行视觉伺服控制.
Nagabandi等人\textsuperscript{\cite{27}}将神经网络动力学模型与模型预测控制相结合, 实现了机器人腿部运动的快速适应.
近期, RT-2\textsuperscript{\cite{8}}将大规模视觉-语言模型与机器人动作空间对齐,
展示了基础模型在机器人控制中的潜力.
...
状态空间模型(SSM)源于控制理论中的线性系统描述\textsuperscript{\cite{29}},
近年来在深度学习领域引起了广泛关注.
Gu等人\textsuperscript{\cite{9}}提出的HiPPO框架通过最优多项式投影解决了连续时间记忆问题,
...
S4D\textsuperscript{\cite{22}}简化了对角SSM的参数化, 直接使用复数对角矩阵,
在保持性能的同时显著降低了实现复杂度.
S5\textsuperscript{\cite{10}}引入MIMO-SSM结构, 通过并行扫描算法实现了更高效的训练.
H3\textsuperscript{\cite{23}}结合了SSM与门控注意力, 在语言建模中取得了与Transformer相当的性能.
Hyena\textsuperscript{\cite{24}}用长卷积替代了注意力机制, 进一步拓展了SSM类方法的设计空间.

Mamba\textsuperscript{\cite{7}}是SSM发展的一个重要里程碑.
...
Mamba-2\textsuperscript{\cite{11}}进一步揭示了SSM与注意力机制之间的结构化状态空间对偶性(structured state space duality),
为统一理解不同序列建模方法提供了新的理论框架.
本文基于S4D风格的对角SSM构建世界模型, 并采用Mamba风格的门控块结构,
在世界模型任务中验证了这一架构组合的有效性.

近期, SSM类方法也开始在机器人领域得到应用.
在策略学习方面, Mamba被用于替代Transformer作为策略网络的主干, 实现了更高效的序列决策\textsuperscript{\cite{40}}.
在轨迹预测方面, SSM被用于预测行人的未来轨迹, 利用其长程建模能力捕捉行人的运动模式\textsuperscript{\cite{41}}.
在世界模型方面, Dreamer\textsuperscript{\cite{12}}和PlaNet\textsuperscript{\cite{19}}等方法使用循环神经网络或潜在动力学模型进行环境建模,
...
知识蒸馏\textsuperscript{\cite{30}}、模型剪枝\textsuperscript{\cite{31}}和量化\textsuperscript{\cite{32}}是常用的模型压缩方法.
网络架构搜索(NAS)\textsuperscript{\cite{33}}也被用于自动设计高效的网络结构.
然而, 这些方法通常是事后(post-hoc)压缩, 可能导致性能损失.

从架构设计角度出发, 门控循环单元(GRU)\textsuperscript{\cite{34}}通过简化LSTM的门控结构减少了参数量.
时间卷积网络(TCN)\textsuperscript{\cite{35}}利用因果卷积和膨胀卷积实现了高效的序列建模.
SSM类方法\textsuperscript{\cite{6,7}}通过线性递推结构实现了$O(T)$的每步推理复杂度和$O(1)$的单步延迟,
在长序列场景下具有显著的计算优势.
...
通过在每个控制时刻求解有限时域优化问题来确定最优动作\textsuperscript{\cite{14}}.
MPC的核心在于动力学模型的准确性: 模型越精确, 控制性能越好.
传统的MPC使用解析模型(如刚体动力学), 但在复杂环境中获取精确的解析模型往往困难重重.

将学习模型与MPC结合是一个活跃的研究方向.
Nagabandi等人\textsuperscript{\cite{27}}展示了神经网络动力学模型在MPC中的有效性.
Chua等人\textsuperscript{\cite{28}}提出了PETS方法, 通过概率集成模型处理模型不确定性.
...
S4D\textsuperscript{\cite{22}}提出将状态矩阵${\bm A}$约束为对角形式:
...
SSM主干网络采用Mamba\textsuperscript{\cite{7}}风格的门控SSM块结构,
结合S4D\textsuperscript{\cite{22}}风格的对角SSM参数化, 在世界模型任务中实现了高效的序列建模.
...
GELU激活函数\textsuperscript{\cite{36}}相比ReLU具有更平滑的梯度特性, 有利于训练稳定性.

\textbf{SSM主干网络.}
采用$L$层SSM块对编码后的序列进行建模.
每个SSM块采用Mamba\textsuperscript{\cite{7}}风格的结构, 包含LayerNorm、对角SSM层、门控机制和残差连接:
\begin{eqnarray}
...
该块结构与Mamba\textsuperscript{\cite{7}}和S5\textsuperscript{\cite{10}}中的设计一致,
门控机制允许模型选择性地保留或更新信息, 类似于LSTM中的门控思想.

对角SSM层采用S4D\textsuperscript{\cite{22}}风格的参数化, 将状态矩阵${\bm A}$约束为对角矩阵,
...
这会导致训练与推理之间的分布不匹配(exposure bias)\textsuperscript{\cite{39}}.
本文通过同时优化单步损失和多步损失来缓解这一问题:
单步损失确保模型在每个时间步的预测准确性,
多步损失则通过展开训练使模型适应自身的预测误差.
实验中$\lambda$和$H$的选择基于验证集上的网格搜索($\lambda \in \{0, 0.1, 0.5, 1.0\}$, $H \in \{4, 8, 16\}$).

优化采用AdamW优化器\textsuperscript{\cite{37}}, 初始学习率为$1\times10^{-3}$,
权重衰减为$1\times10^{-4}$.
学习率调度采用余弦退火策略\textsuperscript{\cite{38}}.
梯度裁剪阈值设为1.0以防止梯度爆炸.
...
\item[4)] Mamba-WM: 基于Mamba\textsuperscript{\cite{7}}选择性SSM的世界模型, 4层, 隐层维度128, 使用官方mamba-ssm库实现(0.28M参数).
\end{enumerate}
...
\item[5)] Mamba-WM与SSM-WM性能接近(MSE=1.28 vs 1.32, $p=0.18$, Cohen's $d{=}0.3$, 小效应), 差异不显著, 验证了对角SSM在世界模型任务中的有效性;
...
\item[2)] SSM-WM相比LSTM-WM提升约6.2\%, 相比Transformer-WM提升约12.8\%, 且差异具有统计显著性(SSM-WM vs LSTM-WM: $p<0.01$, Cohen's $d{=}1.8$, 大效应; SSM-WM vs Transformer-WM: $p<0.001$, Cohen's $d{=}2.4$, 大效应);
...
(2) 推理速度快约16\%(3.8ms vs 4.5ms); 需要指出的是, 该速度差异部分源于实现方式的不同(Mamba使用自定义CUDA算子, SSM-WM使用标准PyTorch FFT), 在相同实现框架下差距可能缩小;
...
(5) 速度优势(合成数据集)和精度优势(MuJoCo数据集)来自不同的实验设置, 尚未在统一实验中同时验证; 未来需要在MuJoCo上进行MPC实验以闭合这一证据链;
(6) MPC实验采用固定计算预算(50次Adam迭代)而非固定精度目标进行对比, 在相同精度要求下各方法的速度差异可能有所不同;
(7) 当前模型仅处理状态-动作序列, 未涉及视觉输入, 限制了其在端到端控制中的应用;
(8) 合成数据集规模较小(100条episode$\times$150步), 每组实验仅使用3个随机种子, 统计功效有限; 本文通过报告Cohen's $d$效应量和95\%置信区间来部分弥补这一不足.
