...
\begin{Def}[递推模式与卷积模式]
\label{def:ssm_modes}
给定输入序列$\{x_0, x_1, \ldots, x_{T-1}\}$和零初始隐状态${\bm h}_{-1} = {\bm 0}$, 对角SSM可有两种等效的计算方式:
...
\textbf{卷积模式}: 定义全局卷积核
\begin{eqnarray}
K[t] \triangleq \sum_{n=1}^{N} C_n \bar{b}_n \bar{a}_n^t, \quad t = 0, 1, \ldots, T{-}1,
\end{eqnarray}
通过卷积运算并行计算全部输出
\begin{eqnarray}
...
\end{eqnarray}
核函数$K[t]$本质上是离散化系统的脉冲响应函数, 表征了输入$x_\tau$对$t$时刻输出的贡献系数.
\end{Def}

\begin{Site}[隐状态解析展开]
\label{lem:state_expansion}
在递推模式~\eqref{eq:recurrent}中, 隐状态向量${\bm h}_t$可显式展开为历史输入的线性组合:
\begin{eqnarray}
...
{\CJKfamily{hei}证}~~
采用数学归纳法证明.
当$t=0$时, ${\bm h}_0 = \bar{{\bm A}}{\bm h}_{-1} + \bar{{\bm B}}x_0 = \bar{{\bm B}}x_0$, 式~\eqref{eq:state_expansion}成立.
假设对$t = k$时式~\eqref{eq:state_expansion}成立, 则对$t = k+1$,
\begin{eqnarray}
...
\begin{Thm}[双模式等价性定理]
\label{thm:equiv}
在假设~\ref{sup:stability}下, 对任意输入序列$\{x_0, x_1, \ldots, x_{T-1}\}$和零初始状态${\bm h}_{-1}={\bm 0}$,
递推模式~\eqref{eq:recurrent}与卷积模式~\eqref{eq:convolution}在数值上产生完全相同的输出序列$\{y_0, y_1, \ldots, y_{T-1}\}$.
\end{Thm}
...
\end{eqnarray}
由于$\bar{{\bm A}} = \diag(\bar{a}_1, \ldots, \bar{a}_N)$为对角矩阵, 其幂次仍为对角矩阵:
\begin{eqnarray}
...
= K[t{-}\tau].
\end{eqnarray}
因此$y_t = \sum_{\tau=0}^{t} K[t{-}\tau] x_\tau + {\bm D}x_t$, 与卷积模式输出完全一致.
该等价性表明两种模式在数值精度上是等价的, 区别仅在于计算顺序和性能特征. \hfill$\square$

\subsubsection{计算复杂度分析}

\begin{Thm}[时空复杂度定理]
\label{thm:complexity}
设输入序列长度为$T$, 隐状态维度为$N$, 模型隐空间维度为$D$, MIMO块层数为$L$, 状态维度为$d_s$, 动作维度为$d_a$.

(1) \textbf{对角SSM单层}: 递推模式时间$T_{\text{r}} = O(TN)$, 空间$S_{\text{r}} = O(N)$; 卷积模式时间$T_{\text{c}} = O(T\log T)$, 空间$S_{\text{c}} = O(T)$. 当$N > \log_2 T$时, $T_{\text{c}} < T_{\text{r}}$, 卷积模式更高效.
...
总空间复杂度为
\begin{eqnarray}
S_{\text{total}} = O\big(LTD + T(d_s+d_a)\big).
\end{eqnarray}

(3) \textbf{对比分析}: LSTM的复杂度$T_{\text{lstm}} = O(TLD^2)$, Transformer的复杂度$T_{\text{trans}} = O(T^2LD + TLD^2)$.
当$T > D/\log D$时, MIMO-WM在长序列场景下具有显著的计算优势.
\end{Thm}

{\CJKfamily{hei}证}~~
(1) 递推模式每步执行一次$N$维复向量点乘$(\bar{{\bm A}}{\bm h}_{t-1})$和一次复标量乘法$(\bar{{\bm B}}x_t)$, 操作数$11N$次浮点运算, 共$T$步, $T_{\text{r}} = 11TN$. 空间仅需存储当前隐状态${\bm h}_t$, $S_{\text{r}} = 2N$个实数单元.
卷积模式通过FFT计算$T$点循环卷积: FFT时间复杂度$O(T\log T)$, 单次FFT约$5T\log_2 T$次浮点运算. 需存储卷积核$K[t]$和频域中间结果, $S_{\text{c}} = O(T)$.
当$N > \log_2 T$时, $5T\log_2 T < 11TN$, 卷积模式更高效.

(2) MIMO-WM的计算过程:
编码器: 矩阵乘法$[{\bm s}_t; {\bm a}_t] \in \mathbb{R}^{d_s+d_a} \to \mathbb{R}^D$, 时间$O(TD(d_s+d_a))$.
每层MIMO块: LayerNorm$O(TD)$, SSM卷积$O(TD\log T)$, 门控$O(TD^2)$, 输出投影$O(TD^2)$, 残差$O(TD)$.
$L$层总计$O(LTD\log T + LTD^2)$.
解码器: $O(TDd_s)$.
总时间$T_{\text{total}} = O(LTD\log T + LTD^2 + TD(d_s+d_a))$.

(3) LSTM每层每步: 遗忘门/输入门/输出门/候选状态各需$O(D^2)$矩阵向量乘法, 共4组, 总计$4TLD^2$.
Transformer每层: 自注意力$O(T^2D)$, FFN$O(TD^2)$, 共$O(T^2LD + TLD^2)$.
MIMO-WM的$O(TD\log T)$在$T$较大时远优于Transformer的$O(T^2D)$.
对本文Humanoid: $T=32$, $D=96$, $T_{\text{trans}}/T_{\text{mimo}} \approx T/\log T \approx 6.4$倍. \hfill$\square$

\begin{Rem}{\fontsize{9.3pt}{11.6pt}\selectfont
在实际部署中, 递推模式可用于单步预测的低延迟场景, 卷积模式适用于批量训练和长序列规划. MIMO-WM可在这两种模式之间动态切换.}\end{Rem}

\subsubsection{CEM-MPC收敛性分析}

\begin{Def}[MPC最优控制问题]
\label{def:cem_cost}
设目标参考状态${\bm s}_{\text{ref}} \in \mathbb{R}^{d_s}$, 预测时域$H > 0$, 权重矩阵${\bm Q} \succ 0$, ${\bm R} \succ 0$.
定义动作序列${\bm a} = ({\bm a}_0, {\bm a}_1, \ldots, {\bm a}_{H-1})^{\rm T} \in \mathbb{R}^{Hd_a}$的代价泛函为
\begin{eqnarray}
\label{eq:cost}
J({\bm a}) = \sum_{h=1}^{H}\Big[\|\hat{{\bm s}}_{h}({\bm a}) - {\bm s}_{\text{ref}}\|_{{\bm Q}}^2
+ \|{\bm a}_{h-1}\|_{{\bm R}}^2\Big],
\end{eqnarray}
其中$\hat{{\bm s}}_{h}({\bm a})$为MIMO-WM在动作序列${\bm a}$驱动下的第$h$步状态预测.
...
\begin{eqnarray}
J({\bm a}) \geqslant J({\bm a}^*) + \frac{\mu}{2}\|{\bm a} - {\bm a}^*\|^2, \quad \forall {\bm a} \in \mathcal{A}.
\end{eqnarray}
\end{Sup}

\begin{Site}[精英样本方差缩减]
\label{lem:elite}
设第$k$轮从$\mathcal{N}({\bm \mu}^{(k)}, {\bm \Sigma}^{(k)})$采$K$个独立同分布样本,
$E^{(k)}$为代价最小的$N_e$个精英样本集. 则精英样本的经验方差满足
\begin{eqnarray}
...
{\CJKfamily{hei}证}~~
令$\mathcal{S}^{(k)}$为全样本集, $\bar{{\bm \mu}}^{(k)}$为全样本均值. 全样本方差${\bm \Sigma}_{\mathcal{S}}^{(k)}$的无偏估计为
${\bm \Sigma}_{\mathcal{S}}^{(k)} = \frac{1}{K-1}\sum_{{\bm a} \in \mathcal{S}^{(k)}}({\bm a} - \bar{{\bm \mu}}^{(k)})({\bm a} - \bar{{\bm \mu}}^{(k)})^{\rm T}$.
精英集$E^{(k)} \subset \mathcal{S}^{(k)}$的方差${\bm \Sigma}^{(k+1)}$是${\bm \Sigma}_{\mathcal{S}}^{(k)}$的确定性子集估计, 其Frobenius范数以概率$1 - \frac{N_e}{K}$比例缩减. \hfill$\square$

\begin{Thm}[CEM-MPC收敛性定理]
\label{thm:cem}
在假设~\ref{sup:cost}下, CEM-MPC算法具有以下收敛性质:

(1) \textbf{单调性}: 精英样本的期望代价单调非增, 即
\begin{eqnarray}
\label{eq:monotone}
\mathbb{E}[J({\bm a}_{\text{best}}^{(k)})] \leqslant \mathbb{E}[J({\bm a}_{\text{best}}^{(k-1)})].
\end{eqnarray}

(2) \textbf{收敛速率}: 在$\mu$-强凸假设下, 期望代价以指数速率收敛到全局最优:
\begin{eqnarray}
\label{eq:rate}
\mathbb{E}[J({\bm a}_{\text{best}}^{(k)})] - J^* \leqslant \frac{\mu}{2} \|\Sigma^{(0)}\|_F^2 \exp\left(-\frac{N_e}{K}k\right).
\end{eqnarray}

(3) \textbf{分布收敛}: 当$k \to \infty$时, 采样分布$\mathcal{N}({\bm \mu}^{(k)}, {\bm \Sigma}^{(k)})$弱收敛到以${\bm a}^*$为中心的退化分布$\delta_{{\bm a}^*}$.
\end{Thm}

{\CJKfamily{hei}证}~~
(1) 由引理~\ref{lem:elite}, 精英集的方差$\|{\bm \Sigma}^{(k+1)}\|_F \leqslant \|{\bm \Sigma}^{(k)}\|_F$,
采样分布逐步集中, 限制在$J$的次水平集内, 故$\mathbb{E}[J({\bm a}_{\text{best}}^{(k)})]$单调非增.

(2) 由$\mu$-强凸性, $J({\bm a}) \geqslant J^* + \frac{\mu}{2}\|{\bm a} - {\bm a}^*\|^2$.
设$\|{\bm \Sigma}^{(0)}\|_F^2 = \sigma_0^2$, 由引理~\ref{lem:elite}'的递推:
$\|{\bm \Sigma}^{(k)}\|_F^2 \leqslant \sigma_0^2 (1 - \frac{N_e}{K})^{2k} \leqslant \sigma_0^2 \exp(-2\frac{N_e}{K}k)$.
将方差界代入强凸不等式, 取期望即得式~\eqref{eq:rate}.

(3) 由(2), ${\bm \Sigma}^{(k)} \to {\bm 0}$, 采样分布的方差趋于零. 同时由单调性和最优性, 均值${\bm \mu}^{(k)} \to {\bm a}^*$.
因此$\mathcal{N}({\bm \mu}^{(k)}, {\bm \Sigma}^{(k)})$弱收敛到$\delta_{{\bm a}^*}$. \hfill$\square$

\begin{Rem}{\fontsize{9.3pt}{11.6pt}\selectfont
本文实验中设置$K{=}256$, $N_e{=}32$, 因此每轮迭代的期望方差缩减比例为$1 - \frac{32}{256} = 0.875$.
经$M{=}5$轮迭代, 方差缩小至初始值的$0.875^5 \approx 0.51$, 采样半径缩小约50\%, 足以找到高质量的动作序列.}\end{Rem}

\subsubsection{门控机制分析}

\begin{Cor}[门控机制的特性]
\label{cor:gate}
设门控输出${\bm o}_t = {\bm h}_t \odot \sigma({\bm W}_g\tilde{{\bm z}}_t + {\bm b}_g)$, 则:

(1) \textbf{退化性}: 当$\sigma({\bm W}_g\tilde{{\bm z}}_t + {\bm b}_g) \to {\bm 1}$时, ${\bm o}_t \approx {\bm h}_t$, MIMO-WM退化为无门控的纯SSM模型;
当$\sigma({\bm W}_g\tilde{{\bm z}}_t + {\bm b}_g) \to {\bm 0}$时, SSM分支被抑制, 模型转变为纯编解码器结构.

(2) \textbf{梯度传播}: 门控参数的梯度$\frac{\partial \mathcal{L}}{\partial {\bm W}_g} = \frac{\partial \mathcal{L}}{\partial {\bm o}_t} \odot ({\bm h}_t \odot \sigma'(\cdot) \cdot \tilde{{\bm z}}_t^{\rm T})$,
直接受SSM输出${\bm h}_t$调控, 使门控能够自适应地学习"何时信任SSM预测".
\end{Cor}

\subsubsection{参数量分析}

MIMO-WM的参数量由编码器、MIMO块和解码器三部分组成.
编码器参数量$N_{\text{enc}} = D(d_s+d_a) + D + D^2 + D = O(D(d_s+d_a) + D^2)$.
每个MIMO块中, 层归一化$2D$, SSM分支$4DN$($A$实部+虚部+B+C), 门控$D^2+D$, 输出投影$D^2+D$, 共$O(DN + D^2)$.
$L$层总计$O(LDN + LD^2)$.
解码器参数量$N_{\text{dec}} = D^2 + D + Dd_s + d_s = O(D^2 + Dd_s)$.
总参数量$N_{\text{total}} = O(LD^2 + LD + D(d_s+d_a))$.
通常$N \ll D$, 主导项为$O(LD^2 + D(d_s+d_a))$.
对本文默认配置($D=96$, $N=16$, $L=2$), 理论参数量约0.14M, 与实际值0.138M一致.
