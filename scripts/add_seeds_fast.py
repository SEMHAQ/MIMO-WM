"""Add seeds 789 and 1024 - FAST version with large batch size to fill GPU."""
...
def train_model(ModelClass, name, kwargs, Xs, Xa, Y, Xv, Xav, Yv, seed):
    ...
    Xv_t = torch.FloatTensor(Xv).to(device)
    Xav_t = torch.FloatTensor(Xav).to(device)
    Yv_t = torch.FloatTensor(Yv).to(device)
...
# Count tasks
total = sum(1 for ds in datasets for m in models for s in NEW_SEEDS if f'seed{s}' not in results.get(f'{m}_{ds}', {}))
print(f'Tasks: {total}, BS={BS}, GPU: {torch.cuda.get_device_name(0)}')
