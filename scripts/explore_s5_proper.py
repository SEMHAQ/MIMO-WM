...
class S5Model(nn.Module):
    ...
    def __init__(self, state_dim, action_dim, d_model=128, d_state=16, n_layers=2):
        super().__init__()
        self.encoder = nn.Sequential(nn.Linear(state_dim + action_dim, d_model), nn.GELU(), nn.Linear(d_model, d_model))
        self.backbone = nn.ModuleList([S5Layer(d_model, d_state) for _ in range(n_layers)])
        self.decoder = nn.Sequential(nn.Linear(d_model, d_model), nn.GELU(), nn.Linear(d_model, state_dim))
...
if __name__ == '__main__':
    ...
    RESULTS_FILE = 'experiments/s5_proper.json'
    ...
    configs = {
        'S5-d128-L1': lambda: S5Model(348, 17, d_model=128, d_state=16, n_layers=1),
        'S5-d128-L2': lambda: S5Model(348, 17, d_model=128, d_state=16, n_layers=2),
        'S5-d96-L2': lambda: S5Model(348, 17, d_model=96, d_state=16, n_layers=2),
        'SimpleSSM': lambda: SimpleSSM(348, 17, d_model=128, d_state=16, n_layers=2),
    }

    print('\n' + '='*60, flush=True)
    print('正确S5实现实验', flush=True)
    print('='*60, flush=True)
