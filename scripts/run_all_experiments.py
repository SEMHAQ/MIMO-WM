"""统一实验脚本 - 基于run_10seeds.py的结构
所有模型使用相同配置，运行所有论文需要的实验"""
...
models = {
    ...
    'Transformer-WM': (TransformerWorldModel, lambda sd, ad: {'state_dim': sd, 'action_dim': ad, 'd_model': 128, 'n_layers': 4}),
    ...
    'FSM-WM': (FSM, lambda sd, ad: {'state_dim': sd, 'action_dim': ad, 'd_model': 128, 'd_state': 16, 'n_layers': 4}),
}
...
for ds_name, ds_cfg in datasets.items():
    ...
    for model_name, (ModelClass, kwargs_fn) in models.items():
        ...
        for seed in SEEDS:
            ...
            with open('experiments/paper_results.json', 'w') as f:
...
for key, seeds in results.items():
    ...
    print(f'{key}: MSE={np.mean(mses):.4f}±{np.std(mses):.4f}, R²={np.mean(r2s):.4f}±{np.std(r2s):.4f}')

print('\nDone! Results saved to experiments/paper_results.json')
