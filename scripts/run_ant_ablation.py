...
if __name__ == '__main__':
    print('加载Ant数据...', flush=True)
    eps_tr = load_eps('data/ant', 'train')
    eps_vl = load_eps('data/ant', 'val')
    m, s = stats(eps_tr)
    Xs, Xa, Y = make_data(eps_tr, T, m, s)
    Xv, Xav, Yv = make_data(eps_vl, T, m, s)
    print(f'Train: {len(Xs)}, Val: {len(Xv)}', flush=True)

    RESULTS_FILE = 'experiments/ant_component_ablation.json'
    ...
    configs = {
        '完整MS-WM': lambda: FullMSWM(105, 8, d_model=96, d_state=8, n_layers=1, window_size=5),
        '只用SSM': lambda: SSMOnly(105, 8, d_model=96, d_state=16, n_layers=2),
        '只用Attention': lambda: AttnOnly(105, 8, d_model=96, n_layers=2, window_size=5),
    }

    print('\n' + '='*60, flush=True)
    print('MS-WM组件消融实验 - Ant', flush=True)
    print('='*60, flush=True)
